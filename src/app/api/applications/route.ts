  ...
  try {
    ...
    if (!applicantId || !jobOrderId) {
      return NextResponse.json(
        { error: 'applicantId and jobOrderId are required' },
        { status: 400 }
      );
    }

    const jobExists = await db.jobOrder.findUnique({
      where: { id: jobOrderId },
    });
    ...
    try {
      ...
      await db.aIAnalysisResult.create({
        data: {
          ...
          matchedSkills: '[]',
          missingSkills: '[]',
          explanation: 'Mock analysis - AI service not configured',
        },
      });

      const applicationWithAI = await db.application.findUnique({
      ...
      return NextResponse.json({ application: applicationWithAI }, { status: 201 });
