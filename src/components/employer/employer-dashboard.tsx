...
export function EmployerDashboard({ showEndorsements, showProfile }: { showEndorsements?: boolean; showProfile?: boolean } = {}) {
  ...
  if (showProfile) {
    return (
      <div className="p-4 md:p-6 max-w-3xl mx-auto">
        <h1 className="text-2xl font-bold mb-6">{t('nav.myProfile')}</h1>
        <Card>
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">{t('employer.dashboard.title')}</h1>
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        {[
          ...
          { label: t('employer.dashboard.accepted'), value: accepted.length, icon: CheckCircle, color: 'bg-emerald-100 text-emerald-700' },
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {endorsements.map(e => {
            ...
            return (
              <Card key={e.id} className={isAccepted ? 'ring-2 ring-emerald-500' : ''}>
                <CardContent className="p-4">
                  {isPending && (
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <Badge className={`text-[10px] ${e.firaStatus === 'approved' ? 'bg-green-100 text-green-700' : e.firaStatus === 'rejected' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        FIRA: {e.firaStatus}
                      </Badge>
                      <Badge className={`text-[10px] ${e.employerStatus === 'accepted' ? 'bg-green-100 text-green-700' : e.employerStatus === 'declined' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'}`}>
                        {e.employerStatus}
                      </Badge>
                    </div>
                  </div>
                  ...
                  {isPending && (
                    <div className="flex gap-2 mt-4 pt-3 border-t">
                      <Button className="bg-emerald-600 hover:bg-emerald-700 min-h-[44px]" onClick={() => handleAction(e.id, 'employer_accept')}>
                        <UserCheck className="h-4 w-4 mr-2" />{t('employer.endorsements.accept')}
                      </Button>
                      <Button variant="outline" className="text-red-600 hover:text-red-700 min-h-[44px]" onClick={() => handleAction(e.id, 'employer_decline')}>
                        <UserX className="h-4 w-4 mr-2" />{t('employer.endorsements.decline')}
                      </Button>
                    </div>
                  )}
                </CardContent>
