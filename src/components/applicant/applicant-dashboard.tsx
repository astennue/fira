...
export function ApplicantDashboard({ showApplications = false }: { showApplications?: boolean }) {
  ...
  const getStatusColor = (status: string) => {
    const map: Record<string, string> = { pending: 'bg-yellow-100 text-yellow-800', accepted: 'bg-green-100 text-green-800', rejected: 'bg-red-100 text-red-800', hired: 'bg-emerald-100 text-emerald-800', withdrawn: 'bg-gray-100 text-gray-800' };
  ...
  if (showApplications) {
    return (
      <div className="p-4 md:p-6 max-w-5xl mx-auto">
        <h2 className="text-2xl font-bold mb-6">{t('applicant.applications.title')}</h2>
        {loading ? (
          ...
          <Card className="p-8 text-center">
            ...
            <Button className="mt-4 bg-emerald-600 hover:bg-emerald-700" onClick={() => navigate('applicant-jobs')}>
              <Search className="h-4 w-4 mr-2" /> {t('applicant.dashboard.browseJobs')}
            </Button>
          </Card>
        ) : (
                ...
                <CardContent className="p-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      {app.matchScore && <Badge variant="outline" className="text-emerald-700">{Math.round(app.matchScore)}%</Badge>}
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-1">{t('applicant.dashboard.title')}, {firstName}!</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
        <Button className="h-14 bg-emerald-600 hover:bg-emerald-700 text-base" onClick={() => navigate('applicant-jobs')}>
          <Search className="h-5 w-5 mr-2" /> {t('applicant.dashboard.browseJobs')}
        </Button>
        <Button variant="outline" className="h-14 text-base" onClick={() => navigate('applicant-profile')}>
          <User className="h-5 w-5 mr-2" /> {t('applicant.dashboard.completeProfile')}
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <Card className="p-6 text-center text-muted-foreground">
          ...
          <p className="text-sm">{t('applicant.applications.empty')}</p>
        </Card>
      ) : (
