  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">{t('applicant.jobs.title')}</h1>
      </div>
            ...
            return (
              <Card key={job.id} className="hover:shadow-md transition-shadow flex flex-col">
                <CardContent className="p-4 flex-1 flex flex-col">
                  ...
                  >
                    {isApplying ? <Loader2 className="h-4 w-4 animate-spin" /> : isApplied ? <><Check className="h-4 w-4 mr-1" />{t('applicant.jobs.applied')}</> : t('applicant.jobs.apply')}
                  </Button>
                </CardContent>
              </Card>
            );
