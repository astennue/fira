...
export function FiraDashboard({ showJobOrders, showEndorsements }: { showJobOrders?: boolean; showEndorsements?: boolean } = {}) {
  ...
  if (showJobOrders) {
    return (
      <div className="p-4 md:p-6 max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-6">{t('nav.jobOrders')}</h1>
          ...
          <div className="space-y-2">
            {jobs.map(j => (
              <Card key={j.id}><CardContent className="p-3 flex justify-between items-center">
                ...
                <Badge className={j.status === 'open' ? 'bg-emerald-100 text-emerald-700 text-xs' : 'bg-gray-100 text-gray-700 text-xs'}>{j.status}</Badge>
              </CardContent></Card>
            ))}
          </div>
        )}
  ...
  if (showEndorsements) {
    return (
      <div className="p-4 md:p-6 max-w-5xl mx-auto">
        <h1 className="text-2xl font-bold mb-6">{t('nav.endorsements')}</h1>
  ...
  return (
    <div className="p-4 md:p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">{t('fira.dashboard.title')}</h1>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        {[
          ...
          { label: t('fira.dashboard.totalEmployers'), value: employers.length, icon: Globe, color: 'bg-purple-100 text-purple-700' },
          { label: t('fira.dashboard.totalJobOrders'), value: jobs.length, icon: Briefcase, color: 'bg-emerald-100 text-emerald-700' },
        ].map(stat => (
          <Card key={stat.label}>
            ...
            {loading ? <Skeleton className="h-12 w-full" /> : pendingAgencies.length === 0 ? (
              ...
              <div key={a.id} className="flex items-center justify-between p-2 rounded-lg border">
                ...
                <div className="flex gap-1 ml-2">
                  <Button size="sm" className="h-8 bg-emerald-600 hover:bg-emerald-700 text-xs" onClick={() => handleApproveAgency(a.id)}><CheckCircle className="h-3 w-3 mr-1" />Approve</Button>
                </div>
              </div>
            ))}
          ...
          <CardContent className="space-y-2">
            {loading ? <Skeleton className="h-12 w-full" /> : pendingEmployers.length === 0 ? (
              ...
              <div key={e.id} className="flex items-center justify-between p-2 rounded-lg border">
                ...
                <Button size="sm" className="h-8 bg-emerald-600 hover:bg-emerald-700 text-xs" onClick={() => handleApproveEmployer(e.id)}>
