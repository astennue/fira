...
export function FiraAgencies() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">{t('nav.agencies')}</h1>
              ...
              <CardContent className="p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className={a.isApproved ? 'bg-green-100 text-green-700 text-xs' : 'bg-yellow-100 text-yellow-700 text-xs'}>{a.isApproved ? 'Approved' : 'Pending'}</Badge>
                    <Button size="sm" variant="outline" className={`h-8 text-xs ${a.isApproved ? 'text-red-600 hover:text-red-700' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                      onClick={() => handleToggle(a.id, !a.isApproved)}>
                      {a.isApproved ? <><XCircle className="h-3 w-3 mr-1" />Reject</> : <><CheckCircle className="h-3 w-3 mr-1" />Approve</>}
                    </Button>
                  </div>
                </div>
              </CardContent>
