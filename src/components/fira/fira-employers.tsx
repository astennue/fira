...
export function FiraEmployers() {
  ...
  return (
    <div className="p-4 md:p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold mb-6">{t('nav.employers')}</h1>
              ...
              <CardContent className="p-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-lg bg-purple-100 flex items-center justify-center"><Globe className="h-5 w-5 text-purple-700" /></div>
                    <div>
                  ...
                  <div className="flex items-center gap-2">
                    <Badge className={e.isApproved ? 'bg-green-100 text-green-700 text-xs' : 'bg-yellow-100 text-yellow-700 text-xs'}>{e.isApproved ? 'Approved' : 'Pending'}</Badge>
                    <Button size="sm" variant="outline" className={`h-8 text-xs ${e.isApproved ? 'text-red-600 hover:text-red-700' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}
                      onClick={() => handleToggle(e.id, !e.isApproved)}>
                      {e.isApproved ? <><XCircle className="h-3 w-3 mr-1" />Reject</> : <><CheckCircle className="h-3 w-3 mr-1" />Approve</>}
                    </Button>
                  </div>
                </div>
              </CardContent>
