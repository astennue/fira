...
export function FiraMatching() {
  ...
  const getProgressColor = (score: number) => score > 80 ? '[&>div]:bg-emerald-500' : score > 60 ? '[&>div]:bg-amber-500' : '[&>div]:bg-red-500';
  ...
  return (
    <div className="p-4 md:p-6 max-w-4xl mx-auto">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold">{t('nav.matching')}</h1>
      ...
      <Card className="mb-6">
        <CardContent className="p-4">
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Button className="bg-emerald-600 hover:bg-emerald-700 min-h-[44px]" onClick={handleMatch} disabled={matching || !selectedJob}>
              {matching ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Matching...</> : <><Star className="h-4 w-4 mr-2" />Find Matches</>}
            </Button>
          </div>
        </CardContent>
      </Card>
      ...
      {!matching && results.length > 0 && (
        <div className="space-y-3">
          ...
          {results.map((r, idx) => (
            <Card key={r.profileId} className={idx === 0 ? 'ring-2 ring-emerald-500' : ''}>
              <CardContent className="p-4">
